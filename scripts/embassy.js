    ...
    bitcoind: {
        async check (effects, configInput) {
            ...
            if (config.advanced.pruning.mode !== "disabled") {
                return {
                    error: 'Pruning must be disabled to use Bitcoin Core directly. To use with a pruned node, set Bitcoin Core to "Internal (Bitcoin Proxy)" instead.'
                };
            }
            return {
                result: null
...
const getConfig = async (effects)=>{
    ...
    ).then((x)=>matchConfig1.unsafeCast(x)
    ).catch((e)=>{
